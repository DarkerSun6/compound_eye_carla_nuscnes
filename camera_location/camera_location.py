...
try:
    ...
    camera_location_re = carla.Transform(carla.Location(x=camera_x, y=camera_y, z=camera_z),carla.Rotation())
    camera = world.spawn_actor(camera_bp, camera_location_re, attach_to =ego_vehicle.get_actor())
    time.sleep(0.1)     
    ...
    Camera_transform_list = [
            [carla.Location(x=0, y=relative_view, z=camera_z),
            carla.Rotation(pitch=0, yaw=-90, roll=0)],

            [carla.Location(x=-relative_view, y=0, z=camera_z),
