...
images_path_1 = 'F:/carla0.9.14/data/aaa/splice3_120/'
images_path = [images_path_1 + str(_) + '/' for _ in range(10)]
...
pohto_splicing_path_1 = 'multi_camera_rgb'
...
def set_image_size(images_path,horizontal,vertical,mul):
    '''
    功能:设置大图的预期尺寸
    horizontal:横排图像数量
    vertical:竖排图像数量
    ...
    image_path = os.path.join(images_path, image_name[1])
    print(image_path)
...
def photo_splicing_3(directories,dim):
    ...
    for j in range(len(directories)):
        # 获取第一个文件夹内的所有图片名称，其他文件夹中的图片与此处的图片名称一致
        # file_names = os.listdir(directories)
        file_names = ['multi_cam_rgb_' + str(_) +'.png' for _ in range(17)]
        # print(directories)
        ...
        for file_name in file_names:
            ...
            pic_path = os.path.join(directories[j], file_name)
            img = cv2.imread(pic_path)
            assert img is not None, f'无法读取图片: {pic_path}'
            pics.append(img)
        ...
        # 添加空白图片在第二行的两端
        blank_image = np.ones((row2[0].shape[0], row2[0].shape[1] // 2, row2[0].shape[2]), dtype=np.uint8)*255
        blank_image_2 = np.ones((row2[0].shape[0], row2[0].shape[1] // 2+1, row2[0].shape[2]), dtype=np.uint8)*255

        # row1 = [blank_image] + row1 +[blank_image]
        row2 = [blank_image] + row2 +[blank_image_2]
        # row3 = [blank_image] + row3 +[blank_image]
        # row4 = [blank_image] + row4 +[blank_image]
        # row5 = [blank_image] + row5 +[blank_image]
        ...
        # 写入合并后的图片
        img_name = 'multi_cam_rgb' + str(j) + '.png'
        outfile_name = os.path.join(pohto_splicing_path, img_name)
        resized = cv2.resize(final_img, dim, interpolation = cv2.INTER_AREA)
...
if __name__ == '__main__':
    # dim = set_image_size(images_path[0],horizontal=4,vertical=5,mul=2)
    dim = set_image_size(images_path[0],horizontal=6,vertical=3,mul=0.5)


    # photo_splicing_2(images_path,dim)
    # print("Photo splicing completed.")

    photo_splicing_3(images_path,dim)
    print("Photo splicing completed.")
